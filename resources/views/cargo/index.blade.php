...
<div class="card-body">
    @can('Crear-Cargo')
    <a href="{{ route('cargos.create') }}"><button type="button" class="btn btn-outline-info">Añadir nuevo
            Cargo</button></a>
    @endcan
    <h5 class="card-title mt-5"><i class="fa-solid fa-table-cells"></i> Tabla Cargos</h5>
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group" role="group">
                            @can('Editar-Cargo')
                            <form action=" {{ route('cargos.edit', ['cargo'=>$item]) }}">
                                @csrf
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                @if ($item->estado == 1)
                                ¿SEGURO QUE QUIERES ELIMINAR EL CARGO {{ $item->nombre }}?
                                @else
                                ¿SEGURO QUE QUIERES RESTAURAR EL CARGO {{ $item->nombre }}?
                                @endif
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                                <form action=" {{ route('cargos.destroy', ['cargo'=>$item->id]) }}" method="post">
                                    @method('DELETE')
                                    @csrf
