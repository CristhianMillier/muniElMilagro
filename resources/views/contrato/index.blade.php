...
<div class="card-body">
    @can('Crear-Contrato')
    <a href="{{ route('contratos.create') }}"><button type="button" class="btn btn-outline-info">Añadir nuevo
            Contrato</button></a>
    @endcan
    <h5 class="card-title mt-5"><i class="fa-solid fa-table-cells"></i> Tabla Contratos</h5>
                ...
                <tr>
                    ...
                    <td>
                        <div class="btn-group" role="group">
                            @can('Editar-Contrato')
                            <form action=" {{ route('contratos.edit', ['contrato'=>$item]) }}">
                                @csrf
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                @if ($item->estado == 1)
                                ¿SEGURO QUE QUIERES ELIMINAR EL CONTRATO {{ $item->nombre }}?
                                @else
                                ¿SEGURO QUE QUIERES RESTAURAR EL CONTRATO {{ $item->nombre }}?
                                @endif
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cerrar</button>
                                <form action=" {{ route('contratos.destroy', ['contrato'=>$item->id]) }}" method="post">
                                    @method('DELETE')
                                    @csrf
